...
func TestAuthenticatorAuthenticateOnceMapsFailures(t *testing.T) {
	tests := []struct {
		name string
		err  error
		want result.ErrorCode
	}{
		{name: "authentication", err: errors.New("535 authentication failed"), want: result.ErrorCodeAuthentication},
		{name: "connection", err: errors.New("dial tcp 127.0.0.1:587: connect: connection refused"), want: result.ErrorCodeConnection},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			out := auth.AuthenticateOnce(context.Background(), strategy.Target{
				...
				Port:     587,
				Protocol: "smtp",
			}, strategy.Credential{Username: "mailer", Password: "wrong"})
